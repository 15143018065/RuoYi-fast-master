        ...
        operate: {
            ...
            archinves: function(id) {
                ...
                if($.common.isEmpty(id) && table.options.type == table_type.bootstrapTreeTable) {
                    ...
                    var url = table.options.archinvesUrl.replace("{id}", row[table.options.uniqueId]);
                    $.modal.open("档案目录管理", url);
                } else {
                    $.modal.openTab("档案目录管理", $.operate.archinvesUrl(id));
                }
            },
            // 档案目录管理，以tab页展现
            archinvesTab: function(id) {
                table.set();
                $.modal.openTab("档案目录管理", $.operate.archinvesUrl(id));
            },
            // 档案目录管理 全屏
            archinvesFull: function(id) {
                table.set();
                var url = "/404.html";
                if ($.common.isNotEmpty(id)) {
                    url = table.options.archinvesUrl.replace("{id}", id);
                } else {
                    if(table.options.type == table_type.bootstrapTreeTable) {
                        var row = $("#" + table.options.id).bootstrapTreeTable('getSelections')[0];
                        if ($.common.isEmpty(row)) {
                        ...
                        url = table.options.archinvesUrl.replace("{id}", row[table.options.uniqueId]);
                    } else {
                        var row = $.common.isEmpty(table.options.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns(table.options.uniqueId);
                        url = table.options.archinvesUrl.replace("{id}", row);
                    }
                }
                $.modal.openFull("档案目录管理", url);
            },
            // 档案目录管理访问地址
            archinvesUrl: function(id) {
                var url = "/404.html";
                if ($.common.isNotEmpty(id)) {
                    url = table.options.archinvesUrl.replace("{id}", id);
                } else {
